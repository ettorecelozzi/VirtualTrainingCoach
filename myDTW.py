...
def dtw(x, y, dist=None, weights=None):
    ...
    return __dtw(x, y, None, dist, weights)


def __dtw(x, y, window, dist, weights):
...
def __dist(sequence1,sequence2,weights=None):
    ...
    if weights is None:
        for i in range(0, len(sequence1)):
            v = np.power(float(sequence1[i][0]) - float(sequence2[i][0]), 2) + np.power(
                float(sequence1[i][1]) - float(sequence2[i][1]), 2)
